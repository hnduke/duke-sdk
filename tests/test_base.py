...
def test_endpoint_no_auth_required_success(mocker, mock_response):
    """
    Test successful request to an endpoint that does not require authentication.
    """

    mock_get = mocker.patch("requests.get")
    mock_get.return_value = mock_response(status_code=200, contents={"success": "true"})
...
def test_endpoint_auth_required_success(mocker, mock_response):
    """
...
def test_endpoint_auth_required_missing_api_key(mocker, mock_response):
    """
...
def test_endpoint_auth_required_incorrect_api_key(mocker, mock_response):
    """
...
def test_endpoint_auth_required_correct_key_but_endpoint_does_not_exist(
...
def test_endpoint_auth_required_correct_key_but_server_error_response(
    mocker, mock_response
):
    """
    Test request to an endpoint that requires authentication, API key is correct,
